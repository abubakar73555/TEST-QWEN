...
function Register({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/dashboard');
    } catch (err) { setError(err.response?.data?.error || 'خطأ في التسجيل'); }
  ...
  return (
    <div className="container" style={{maxWidth:'400px',paddingTop:'50px'}}>
      <div className="card">
        <h2 className="text-center mb-4" style={{color:'#667eea'}}>إنشاء حساب جديد</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary" style={{width:'100%'}} disabled={loading}>{loading?'جاري...':'تسجيل'}</button>
        </form>
        <p className="text-center mt-4">لديك حساب؟ <Link to="/login" style={{color:'#667eea'}}>دخول</Link></p>
      </div>
    </div>
  );
}
